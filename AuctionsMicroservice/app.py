...
def get_auctions(auth_response=None):
    try:
        response = requests.get(config.dbmanagers.auction + '/auction/status/1', timeout=5, verify=False)
...
def create_auction(auth_response=None):
    try:
        ...
        try:
            gacha_res = requests.get(f"{config.dbmanagers.gacha}/gachacollection/item/{data["gachaCollectionId"]}", timeout=5, verify=False)
        ...
        active_auction = requests.get(config.dbmanagers.auction + f'/auction/{current_user_id}/{data["gachaCollectionId"]}/1', timeout=5, verify=False)
        try:
        ...
        response = requests.post(config.dbmanagers.auction + '/auction', json=new_auction, timeout=5, verify=False)
...
def bid_on_auction(auction_id, auth_response=None):
    try:
        ...
        response = requests.get(config.dbmanagers.auction + f'/auction/{auction_id}', timeout=5, verify=False)
        response.raise_for_status()
        ...
        auction_bids_resp = requests.get(config.dbmanagers.auction + f'/auctionbid/auction/{auction_id}', timeout=5, verify=False)
...
def auction_player_history(auth_response=None):
    try:
        current_user_id = auth_response.json()["userId"]
        data = request.get_json()
        response = requests.get(config.dbmanagers.auction + f'/auction/{current_user_id}/2', timeout=5, verify=False)
...
def check_auction(auction) -> bool:
    ...
    collection = requests.get(config.dbmanagers.gacha+f"/gachacollection/{auction.gachaCollectionId}", timeout=5, verify=False)
...
def close_auction(auction, headers) -> bool:
    ...
    requests.put(f"{config.dbmanagers.auction}/auction/{auction["id"]}", json=auction, headers=headers, timeout=5, verify=False)

    print("SET TO PASSED", flush=True)

    # Get all bids for the auction
    bids = requests.get(f"{config.dbmanagers.auction}/auctionbid/auction/{auction["id"]}", headers=headers, timeout=5, verify=False).json()
    if not bids:
    ...
    resp = requests.post(f"{config.dbmanagers.transaction}/auctiontransaction", headers=headers, json=transaction, timeout=5, verify=False)
    ...
    requests.delete(f"{config.dbmanagers.gacha}/gachacollection/{auction['gachaCollectionId']}", timeout=5, verify=False)
...
def get_all_auctions(auth_response=None):
    ...
    try:
        response = requests.get(config.dbmanagers.auction + f'/auction', timeout=5, verify=False)
...
def update_auction(auction_id, auth_response=None):
    ...
    try:
        ...
        res = requests.get(f"{config.dbmanagers.auction}/auction/{auction_id}", timeout=5, verify=False)
        res.raise_for_status()
        ...
        res = requests.put(f"{config.dbmanagers.auction}/auction/{auction_id}", json=auction, timeout=5, verify=False)
        res.raise_for_status()
...
def history(auth_response=None):
    ...
    try:
        ...
        response = requests.get(config.dbmanagers.auction + f'/auction/2', timeout=5, verify=False)
...
def user_history(user_id, auth_response=None):
    ...
    try:
        ...
        auction = requests.get(config.dbmanagers.auction + f'/auction/user/{user_id}/2', timeout=5, verify=False)
