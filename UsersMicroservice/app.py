...
def delete_player():
    ...
    if delete_response.status_code == 200:
        delete_response = requests.delete(f'{config.services.authmicroservice}/api/player/{user_id}', verify=False)  
        if delete_response.status_code ==200:
...
def get_players():
    ...
    if auth_response.status_code != 200:
        return make_response(auth_response.json(), auth_response.status_code) 
    response = requests.get(f'{config.dbmanagers.user}/user', verify=False)
    if response.status_code == 200:
        return make_response(jsonify(response.json()), 200)
    else:
