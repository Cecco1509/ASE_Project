...
def get_player_transaction_history_admin(user_id, auth_response=None):
    try:
        response = requests.get(config.dbmanagers.transaction + f"/auctiontransaction/user/{user_id}")
...
def get_player_transaction_history(auth_response=None):
    try:
        response = requests.get(config.dbmanagers.transaction + f"/auctiontransaction/user/{auth_response["userId"]}")
        response.raise_for_status()
